...
BEGIN
  ...
  -- Só abre janela para mensagens inbound reais da Meta (com meta_message_id)
  IF v_dir <> 'inbound'
     OR v_instance <> 'meta_official'
     OR NEW.phone IS NULL
     OR NEW.meta_message_id IS NULL
     OR trim(NEW.meta_message_id) = '' THEN
    RETURN NEW;
  END IF;
...
-- Limpa janelas abertas por inbounds sem meta_message_id (inserts manuais/teste)
DELETE FROM public.whatsapp_conversation_windows w
WHERE w.source = 'inbound_message'
  AND NOT EXISTS (
    SELECT 1 FROM public.chat_messages cm
    WHERE cm.whatsapp_instance_name = 'meta_official'
      AND lower(trim(cm.message_direction)) = 'inbound'
      AND cm.meta_message_id IS NOT NULL
      AND trim(cm.meta_message_id) <> ''
      AND regexp_replace(cm.phone, '[^0-9]', '', 'g') = w.phone_e164
